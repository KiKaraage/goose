...
const TEST_IMAGE_BYTES: &[u8] = include_bytes!("test_assets/test_image.png");

pub static FAKE_SCREENSHOT: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| BASE64.encode(TEST_IMAGE_BYTES));
...
impl McpFixtureServer {
    ...
    #[tool(description = "Get a screenshot")]
    fn get_screenshot(&self) -> Result<CallToolResult, McpError> {
        Ok(CallToolResult::success(vec![Content::image(
            FAKE_SCREENSHOT.as_str(),
            "image/png",
        )]))
    }
}
...
impl ServerHandler for McpFixtureServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            ...
            instructions: Some("Test server with get_code and get_screenshot tools.".into()),
        }
    }
}
